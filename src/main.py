...
class Bot(commands.Bot):
    ...
    def __init__(self):
        ...
        super().__init__(token='67qpusn45w05rniu5t5gj83c0a1p9d', prefix='?', initial_channels=['gaules', 'yoda', 'summit1g', 'casimito', 'mch_agg', 'ale_apoka', 'hasanabi', 'mizkif', 'elspreen', 'esl_csgo'])
    ...
    async def event_message(self, message, initial_channels=None):
        ...
        data_event_message = (message.author.name, message.channel.name, message.content,
                              message.timestamp.strftime("%Y-%m-%d %H:%M:%S"), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
